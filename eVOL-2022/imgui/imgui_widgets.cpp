...
bool ImGui::SquareButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderNavHighlight(bb, id);
    RenderFrame(bb.Min, bb.Max, bg_col, true, g.Style.FrameRounding);
    RenderSquare(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), text_col, dir);
...
bool ImGui::SquareButton(const char* str_id, ImGuiDir dir)
{
    float sz = GetFrameHeight();
    return SquareButtonEx(str_id, dir, ImVec2(sz, sz), 0);
}

bool ImGui::ArrowButtonDouble(const char* str_id, ImGuiDir dir)
{
    float sz = GetFrameHeight();
    return ArrowButtonExDouble(str_id, dir, ImVec2(sz * 2, sz), 0);
}
...
bool ImGui::ImageButtonID(int buttonid, ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        float x1 = window->Pos.x + 3;
        float x2 = window->Pos.x + window->Size.x - 3;
        ...
        for (int i = 0; i < windowsize; i++)
        {
            ...
            window->DrawList->AddLine(ImVec2(bb.Min.x + windowsize / windowsize * i, bb.Min.y), ImVec2(bb.Min.x + windowsize / windowsize * (i + 1), bb.Min.y), GetColorU32(ImGuiCol_Separator));
            ImGui::PopStyleColor();
        }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float expected_w = CalcItemWidth() + 1;
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* v, int components, float v_speed, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    BeginGroup();
    PushID(label);
    PushMultiItemsWidths(components, CalcItemWidth() + 1);
