...
void GetSpreadOffset(unsigned int seed, int future, float *inangles, float *outangles)
{
	if (cvar.rage_nospread_method == 0)
	{
		...
		if (PitchInput > 1 && PitchInput < -1)
		{
			...
			QAngle qvTempPitch(0, TempPitch, 0);
			VectorRotate(RotatedPitch, UnRotatedPitch, qvTempPitch);
	...
	else
	{
		...
		QNewAngles = ToEulerAngles(vecDir);
		...
		QNewAngles = ToEulerAngles(vecDir, &vecUp);
