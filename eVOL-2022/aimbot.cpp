...
void SmoothAimAngles(QAngle MyViewAngles, QAngle AimAngles, QAngle& OutAngles, float Smoothing, bool bSpiral, float SpiralX, float SpiralY)
{
	...
	if (bSpiral && SpiralX != 0 && SpiralY != 0)
	{
		...
		Vector vSpiral(vecViewAngleDelta.y / SpiralX, vecViewAngleDelta.x / SpiralY, 0.0f);
		VectorAdd(vecViewAngleDelta, vSpiral, vecViewAngleDelta);
	}

	if (!isnan(Smoothing))
	{
		vecViewAngleDelta.x /= Smoothing;
		vecViewAngleDelta.y /= Smoothing;
		vecViewAngleDelta.z /= Smoothing;
	}
...
void LegitSelect(playeraim_t Aim, Vector vecFOV, float& flBestFOV, float flSpeedScaleFov, float& flSpeed)
{
	...
	for (model_aim_select_t& Model_Selected : Model_Aim_Select)
	{
		...
		if ((cvar.bypass_trace_legit && tr.fraction == 1 && !detect) || (!cvar.bypass_trace_legit && detect == Aim.index))
		{
			...
			float fov = AngleBetween(vecFOV, vDistance);
			if (fov < flBestFOV)
			{
				flBestFOV = fov;
				...
				if (flSpeedScaleFov > 0 && flSpeedScaleFov <= 100 && (VectorIsZero(g_Local.vPunchangle)) && !isnan(Aim.PlayerAimHitbox[Model_Selected.numhitbox].HitboxFOV))
	...
	if (!hitboxselected)
	{
		...
		if ((cvar.bypass_trace_legit && tr.fraction == 1 && !detect) || (!cvar.bypass_trace_legit && detect == Aim.index))
		{
			...
			if (fov < flBestFOV)
			{
				...
				if (flSpeedScaleFov > 0 && flSpeedScaleFov <= 100 && VectorIsZero(g_Local.vPunchangle) && !isnan(Aim.PlayerAimHitbox[HeadBox[Aim.index]].HitboxFOV))
...
void LegitAimbot(struct usercmd_s* cmd)
{
	...
	if (!VectorIsZero2D(g_Local.vPunchangle))
		bSpeedSpiral = false;
	...
	if (iTargetLegit)
	{
		...
		if (g_Engine.GetViewModel()->curstate.sequence == 0 && vm && cvar.legit_autoscope && IsCurWeaponSniper() && g_Local.iFOV == 90)
		...
		else
		{
			...
			if (flPSilent > 0 && flPSilent <= 1 && CanAttack())
			{
				...
				float fov = AngleBetween(vecPsilentFOV, vDistance);
			...
			if (bAttack)
			{
				...
				cmd->viewangles.x = QSmoothAngles.x;
				cmd->viewangles.y = QSmoothAngles.y;
				cmd->viewangles.z = QSmoothAngles.z;
	...
	{
	    if (cvar.legit_autoscope && IsCurWeaponSniper() && g_Local.iFOV != 90)
			cmd->buttons |= IN_ATTACK2;
		if (flBlockAttackAfterKill > 0 && GetTickCount() - dwBlockAttack < flBlockAttackAfterKill)
			cmd->buttons &= ~IN_ATTACK;
	}
}
...
void KnifeAimBot(struct usercmd_s* cmd)
{
	...
	if (iTargetKnife)
	{
		...
		float fDistance = (vAimOriginKnife - (pmove->origin + pmove->view_ofs)).Length();
		if (fDistance < dist)
		{
			if (CanAttack())
			{
				...
				{
					cmd->viewangles.x = QAimAngles.x;
					cmd->viewangles.y = QAimAngles.y;
					cmd->viewangles.z = QAimAngles.z;
