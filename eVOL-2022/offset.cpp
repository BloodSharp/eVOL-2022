...
void AutoOffset::Error(const char* fmt, ...)
{
	va_list va_alist;
	char buf[256];
...
DWORD FindPattern(LPCSTR pattern, LPCSTR mask, DWORD start, DWORD end, DWORD offset)
{
...
DWORD FindPattern(LPCSTR pattern, DWORD start, DWORD end, DWORD offset)
{
	if (start > end)
	{
		DWORD reverse = end;
...
DWORD FindPush(DWORD start, DWORD end, LPCSTR Message)
{
