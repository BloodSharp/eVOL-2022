...
std::deque<spawnorigin_t> SpawnOrigin;
std::deque<spawndeath_t> SpawnDeath;
	...
	if (cvar.visual_spawn_scan && cvar.visual_spawn_points)
	{
		for (unsigned int i = 0; i < SpawnDeath.size(); i++)
		{
			...
			if (!player || !(lstrlenA(player->name) > 0))
				continue;
			if (strcmp(SpawnDeath[i].name, player->name))
				continue;
			if (g_Player[ent->index].iTeam == 0)
				continue;
			if (VectorIsZero(ent->curstate.mins) || VectorIsZero(ent->curstate.maxs))
				continue;
			if (ent->curstate.gaitsequence != GAITSEQUENCE_STAND)
				continue;
			float fDistance = (ent->origin - ent->curstate.origin).Length();
			float distance = fDistance;
			if (distance != 0.0f)
				continue;
			...
			{
				float fDistance = (SpawnOrigins.Origin - tr.endpos).Length();
				float distance = fDistance;
	...
	while (ifs.good())
	{
		...
		if (sscanf(buf, "Spawn: %f,%f,%f", &x, &y, &z))
		{
			if (x != 0 && y != 0 && z != 0)
			{
				...
				{
					float fDistance = (Spawns.Origin - origin).Length();
					float distance = fDistance;
